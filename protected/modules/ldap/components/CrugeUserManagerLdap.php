...
define("CRUGEUSERSTATE_NOTACTIVATED_LDAP", 0);
define("CRUGEUSERSTATE_ACTIVATED_LDAP", 1);
define("CRUGEUSERSTATE_SUSPENDED_LDAP", 2);

define("CRUGEFIELDTYPE_TEXTBOX_LDAP", 0);
define("CRUGEFIELDTYPE_TEXTAREA_LDAP", 1);
define("CRUGEFIELDTYPE_BOOLEAN_LDAP", 2);
define("CRUGEFIELDTYPE_LISTBOX_LDAP", 3);

define("CRUGE_ACTIVATION_OPTION_INMEDIATE_LDAP", 0);
define("CRUGE_ACTIVATION_OPTION_EMAIL_LDAP", 1);
define("CRUGE_ACTIVATION_OPTION_MANUAL_LDAP", 2);


class CrugeUserManagerLdap
{
    ...
    */
    ...
    {
        $stAr = array();
        for ($i = CRUGEUSERSTATE_NOTACTIVATED_LDAP; $i <= CRUGEUSERSTATE_SUSPENDED_LDAP; $i++) {
    ...
    {
        switch ($state) {
            case CRUGEUSERSTATE_NOTACTIVATED_LDAP:
                return CrugeTranslator::t("Cuenta sin Activar");
            case CRUGEUSERSTATE_ACTIVATED_LDAP:
                return CrugeTranslator::t("Cuenta Activada");
            case CRUGEUSERSTATE_SUSPENDED_LDAP:
    ...
    {
        $stAr = array();
        for ($i = CRUGEFIELDTYPE_TEXTBOX_LDAP; $i <= CRUGEFIELDTYPE_LISTBOX_LDAP; $i++) {
    ...
    {
        switch ($fieldType) {
            case CRUGEFIELDTYPE_TEXTBOX_LDAP:
                return CrugeTranslator::t("TextBox");
            case CRUGEFIELDTYPE_TEXTAREA_LDAP:
                return CrugeTranslator::t("TextArea");
            case CRUGEFIELDTYPE_BOOLEAN_LDAP:
                return CrugeTranslator::t("CheckBox");
            case CRUGEFIELDTYPE_LISTBOX_LDAP:
    ...
    {
        $stAr = array();
        for ($i = CRUGE_ACTIVATION_OPTION_INMEDIATE_LDAP; $i <= CRUGE_ACTIVATION_OPTION_MANUAL_LDAP; $i++) {
    ...
    {
        switch ($state) {
            case CRUGE_ACTIVATION_OPTION_INMEDIATE_LDAP:
                return CrugeTranslator::t("Activacion inmediata");
            case CRUGE_ACTIVATION_OPTION_EMAIL_LDAP:
                return CrugeTranslator::t("Activar mediante correo");
            case CRUGE_ACTIVATION_OPTION_MANUAL_LDAP:
    ...
    */
    ...
    {
        $user = CrugeFactoryLdap::get()->getICrugeStoredUserByUsername($username);
        if (($boolAndLoadFields == true) && ($user != null)) {
            $this->loadUserFields($user);
        }
        return $user;
    ...
    */
    ...
    /*
        ...
        solo aplica si el estado del modelo es: CRUGEUSERSTATE_NOTACTIVATED_LDAP
    */
    public function activateAccount(ICrugeStoredUser $user)
    {
        if ($user->state != CRUGEUSERSTATE_NOTACTIVATED_LDAP) {
            return false;
        }
        $user->state = CRUGEUSERSTATE_ACTIVATED_LDAP;
    ...
    */
    public function save(ICrugeStoredUser $user, $scenario = 'internal')
    {
        ...
        if (($user->scenario == 'insert') || ($user->scenario == 'update')) {
            $filtro = CrugeFactoryLdap::get()->getUserFilter();
            if ($filtro != null) {
    ...
    */
    ...
    {
        $epwd = $newPassword;
        if (CrugeUtilLdap::config()->useEncryptedPassword == true) {
            $epwd = CrugeUtilLdap::hash($newPassword);
        }
        $user->password = $epwd;
    }
    ...
    */
    ...
    {
        return CrugeFactory::get()->getICrugeFieldCreate(CRUGEFIELDTYPE_TEXTBOX_LDAP);
    }
    ...
    */
    public function getInputField($model, ICrugeField $field)
    {
        ...
        //
        $arOpt = array();
        if ($field->fieldtype == CRUGEFIELDTYPE_LISTBOX_LDAP) {
    ...
    */
    ...
    /**Clase Customizada cesar Diaz
    **/

      public function createNewUserPassword($mapped_values,$role_name='default', $bool_send_email=false)
    {
        // para crear el usuario se requiere como minimo el email
        // si el username no fue provisto se creará uno en base
        // al email:
        ...
        $this->activateAccount($user);
        // ahora a ponerle una clave
        $this->changePassword($user, $user->password);
        // revisa que no duplique
        $_prev = CrugeFactoryLdap::get()->getICrugeStoredUser($user->username);
        if($_prev != null)
            throw new CrugeException("nombre usuario duplicado.");
        $_prev = CrugeFactoryLdap::get()->getICrugeStoredUser($user->email);
        if($_prev != null)
            throw new CrugeException("correo duplicado.");
        // guarda usando el API. No pasa por filtros. se reparara a futuro.
        if ($this->save($user)) {
